...
if (charttype == '' && charttype == null)
    ...
    Chart =
            Highcharts.chart(Container, {
                ...
                credits: {
                    enabled: true,
                    text:"Dashboard",// we can do our branding here
                    position:{
                        align:"left",
                       x:10,
                       y:0
...
function PopulateDataTable()
{
   ...
   /* $('#Main_datatable').DataTable( {
    ...
    if(dtbl2.length){
        $("#sec_datatable").html(dtbl2);
    $('#sec_datatable').DataTable( {
        dom: 'Bfrtip',
        buttons: [
            'copy', 'csv', 'excel', 'pdf', 'print'
        ]
